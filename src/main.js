...
async function sendToGemini(isOpening) {
  ...
  try {
    ...
    if (!res.ok) {
      addBubble("⚠️ Er ging iets mis:\n" + (data.error?.message || ("Fout " + res.status)), "ai");
      reEnable();
      return;
    }

    let aiText = data.candidates?.[0]?.content?.parts?.[0]?.text || "Hmm, ik kreeg geen antwoord terug. Probeer het nog eens.";
    const clean = extractStep(aiText);
    history.push({ role: "model", text: aiText });
    addBubble(clean, "ai");
  } catch (err) {
    hideTyping();
    addBubble("⚠️ Geen verbinding met de Gemini API.", "ai");
  }
  reEnable();
}
