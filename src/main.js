import SAM_IMG from '../ft/Sam.png';
import JIJ_IMG from '../ft/Jij.png';
import SAM_GD_IMG from '../ft/Sam (Guyiday).png';
import JIJ_GD_IMG from '../ft/Jij (Guyiday).png';
import SAM_WL_IMG from '../ft/Sam (White Label).png';
import JIJ_WL_IMG from '../ft/Jij (White Label).png';

const AVATAR_AI_UNUSED = "data:image/jpeg;base64,";
const AVATAR_ME = "data:image/jpeg;base64,";

const DEFAULT_PROMPT = `Je bent het informele, nuchtere en directe HR-maatje voor medewerkers. Je bent een ster in het herkennen van patronen en je praat alsof je naast de medewerker bij de koffieautomaat staat. Je doel is om een dialoog te voeren die niet als 'werk' voelt, maar als een laagdrempelige stap richting een betere werksfeer. Je helpt de medewerker om input te leveren voor de aankomende evaluatiesessie. Je laat je niet afleiden door onderwerpen die niks met het werk of de werksfeer te maken hebben; je bewaakt strikt de focus van het gesprek.

DOEL & EXPORT-STRUCTUUR (Achter de schermen)
Aan het einde van het gesprek (in Stap 6) sla je de input van de medewerker intern anoniem op volgens deze structuur:
- Hoofdthema / Pijnpunt: [Wat is de kern? (Kan ook een positief thema zijn als er niks speelt)]
- Impact: [Hoe beïnvloedt dit het werk/team?]
- Succesmoment: [De positieve afsluiter]

CONVERSATIE-LOGICA: HET GESPREKSPROTOCOL

Stap 1: De Opening (Het 'Nu' moment)
Begin het gesprek ALTIJD exact zo:
"Hé! Alles goed met je? Even voordat we het gesprek induiken: alles is vertrouwelijk en je kunt stoppen wanneer je wilt.
Is er iets waar je op dit moment, vandaag of deze week, tegenaan loopt in je werk of binnen het team?"
[Voeg marker [STEP:1] toe]

Stap 2: Verdieping van het 'Nu'
Vraag door op het antwoord van de medewerker. Gebruik open vragen (Hoe, wat, wanneer) om de kern en de impact helder te krijgen.
- ALS er een pijnpunt is: Graaf door tot de angel eruit is.
- ALS de medewerker aangeeft dat er niks speelt / alles goed gaat: Vraag door op wat er momenteel zo lekker loopt en waarom het zo goed gaat (Vind de succesfactoren).
[Voeg marker [STEP:2] toe]

Stap 3: De Check (Begrip toetsen)
Voordat je naar de stelling gaat, check je eerst of je het goed hebt begrepen. Spiegel wat de medewerker heeft gezegd in je eigen (informele) woorden en vraag of dit klopt.
(Bijv: "Als ik het zo hoor, zit de frustratie dus vooral in [X]..." OF "Als ik het goed begrijp, zit je dus in een lekkere flow omdat [Y]... Klopt dat een beetje?")
[Voeg marker [STEP:3] toe]

Stap 4: De Neutrale Stelling (Objectiveren)
Vertaal de situatie naar maximaal 2 krachtige, anonieme en objectieve stellingen (dit mag een verbeterpunt óf een succes-stelling zijn, afhankelijk van de input). Vermijd moeilijke HR-termen en houd het luchtig.
Check: "Ik heb een samenvatting gemaakt van het belangrijkste thema van je maand: [Stelling]. Slaat dit de spijker op zijn kop?"
[Voeg marker [STEP:4] toe]

Stap 5: De Peak-End (De Neuromarketing-afsluiter)
Forceer een positieve afsluiting voor de 'Peak-End-Rule':
"Laatste vraag: wat was deze maand echt een succesmoment? Wat ging er zo goed dat je er met een goed gevoel op terugkijkt?"
[Voeg marker [STEP:5] toe]

Stap 6: Afronding
Eindig informeel: "Top, ik sla dit anoniem op. We gaan hiermee aan de slag tijdens de volgende evaluatiesessie. Lekker gewerkt!"
[Voeg marker [STEP:6] toe]

BUITEN-SCOPE PROTOCOL (Focus bewaken)
Als de medewerker een vraag stelt of over een onderwerp begint dat niks met werk, het team, de werksfeer of de evaluatie te maken heeft (bijvoorbeeld random vragen over hobby's, eten, algemene kennis of koetjes en kalfjes):
- Instructie: Ga NIET in op het onderwerp. Kap het vriendelijk maar direct af en stuur direct terug naar het doel van de bot.
- Voorbeeldreactie: "Haha, leuke vraag of opmerking, maar ik ben je HR-maatje, geen Wikipedia! Laten we het snel weer over jou en je werk hebben. Waar liep je deze week tegenaan?"
- Actie: Blijf in de stap waar je op dat moment zat en herhaal de vraag van die stap indien nodig. Verander de [STEP:x] marker niet.

ESCALATIE-PROTOCOL (Veiligheid)
Bij meldingen van grensoverschrijdend gedrag (intimidatie, pesten, discriminatie) stop je het protocol direct, ongeacht in welke stap je zit.
- Instructie: Zeg: "Dit klinkt als een situatie waar ik je als chatbot niet direct bij kan helpen. Ik adviseer je om dit te bespreken met je leidinggevende. Die is hiervoor het aanspreekpunt."
- Uitzondering: Als de medewerker aangeeft dat de leidinggevende juist het probleem is, zeg dan: "Ik begrijp dat dit lastig is. In dat geval is het verstandig om hierover te praten met de vertrouwenspersoon van de organisatie."

BELANGRIJKE SPREEK- EN FORMATTING-INSTRUCTIE:
- Voeg AAN HET EINDE van ELK bericht dat je stuurt de marker toe in het exacte formaat: [STEP:x] waarbij x het nummer is van de stap waar je NU in zit (1 t/m 6).
- Schrijf NOOIT zelf "Stap X" in je zichtbare tekst voor de gebruiker.`;

const STORAGE_KEY = "aibuddy_prompt";
let systemPrompt = DEFAULT_PROMPT;
let history = [];
let currentStep = 1;
const TOTAL_STEPS = 6;
let appPassword = '';
let apiEnabled = true;

const $ = (id) => document.getElementById(id);

/* ---------- Login ---------- */
function initLogin() {
  const btn = $("loginBtn");
  const input = $("passwordInput");

  async function doLogin(pw) {
    btn.disabled = true;
    btn.textContent = 'Bezig…';

    const res = await fetch("/api/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ password: pw, checkOnly: true, contents: [], systemPrompt: '' })
    });

    if (res.status === 401) {
      $("loginError").textContent = "Onjuist wachtwoord.";
      btn.disabled = false;
      btn.textContent = 'Inloggen';
      return;
    }

    appPassword = pw;
    $("loginScreen").classList.add("hidden");
    $("app").classList.remove("hidden");
    loadPrompt();
    startConversation();
  }

  /* Check of wachtwoord vereist is. Zo niet: direct app starten */
  fetch("/api/chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ password: '', checkOnly: true, contents: [], systemPrompt: '' })
  }).then(res => {
    if (res.ok) {
      doLogin('');
    } else {
      $("loginScreen").classList.remove("hidden");
      $("app").classList.add("hidden");
    }
  });

  function tryLogin() {
    const pw = input.value.trim();
    if (!pw) return;
    doLogin(pw);
  }

  btn.addEventListener("click", tryLogin);
  input.addEventListener("keydown", (e) => { if (e.key === "Enter") tryLogin(); });
}

function loadPrompt() {
  try {
    const saved = localStorage.getItem(STORAGE_KEY);
    if (saved && saved.trim()) systemPrompt = saved;
  } catch (e) {}
  $("promptArea").value = systemPrompt;
}

function avatarEl(who) {
  const isGD = $("app").classList.contains("theme-gd");
  const isWL = $("app").classList.contains("theme-wl");
  const img = document.createElement("img");
  img.className = "avatar";
  if (who === "ai") {
    img.src = isGD ? SAM_GD_IMG : isWL ? SAM_WL_IMG : SAM_IMG;
    img.alt = "Sam";
  } else {
    img.src = isGD ? JIJ_GD_IMG : isWL ? JIJ_WL_IMG : JIJ_IMG;
    img.alt = "Jij";
  }
  return img;
}

function nowTime() {
  return new Date().toLocaleTimeString('nl-NL', { hour: '2-digit', minute: '2-digit' }) + 'u';
}

function addBubble(text, who) {
  const row = document.createElement("div");
  row.className = "row " + who;
  const av = avatarEl(who);
  const bub = document.createElement("div");
  bub.className = "bubble";

  const hdr = document.createElement("div");
  hdr.className = "bubble-header";
  hdr.textContent = (who === "ai" ? "Sam" : "Jij") + " , " + nowTime();

  const content = document.createElement("div");
  content.textContent = text;

  bub.appendChild(hdr);
  bub.appendChild(content);

  if (who === "ai") { row.appendChild(av); row.appendChild(bub); }
  else { row.appendChild(bub); row.appendChild(av); }
  $("chat").appendChild(row);
  scrollDown();
  return row;
}

function showTyping() {
  const row = document.createElement("div");
  row.className = "row ai"; row.id = "typingRow";
  row.appendChild(avatarEl("ai"));
  const bub = document.createElement("div");
  bub.className = "bubble";
  bub.innerHTML = '<div class="typing"><span></span><span></span><span></span></div>';
  row.appendChild(bub);
  $("chat").appendChild(row);
  scrollDown();
}
function hideTyping() { const t = $("typingRow"); if (t) t.remove(); }

function scrollDown() {
  const c = $("chat");
  setTimeout(() => { c.scrollTop = c.scrollHeight; }, 30);
}

function updateProgress(step) {
  currentStep = Math.max(1, Math.min(TOTAL_STEPS, step));
  $("progressFill").style.width = (currentStep / TOTAL_STEPS) * 100 + "%";
}

function extractStep(text) {
  const m = text.match(/\[STEP:\s*(\d)\s*\]/i);
  if (m) updateProgress(parseInt(m[1], 10));
  return text.replace(/\[STEP:\s*\d\s*\]/ig, "").trim();
}

async function sendToGemini(isOpening) {
  showTyping();
  $("sendBtn").disabled = true;
  $("msgInput").disabled = true;

  const contents = history.map(h => ({ role: h.role, parts: [{ text: h.text }] }));
  if (isOpening) {
    contents.push({ role: "user", parts: [{ text: "Start het gesprek volgens stap 1 (De Opening)." }] });
  }

  if (!apiEnabled) {
    hideTyping();
    addBubble("[TEST] API staat uit. Dit is een nep-antwoord. [STEP:1]", "ai");
    reEnable();
    return;
  }

  try {
    const res = await fetch("/api/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ contents, systemPrompt, password: appPassword })
    });
    const data = await res.json();
    hideTyping();

    if (!res.ok) {
      addBubble("⚠️ Er ging iets mis:\n" + (data.error || ("Fout " + res.status)), "ai");
      reEnable();
      return;
    }

    let aiText = data.text || "Hmm, ik kreeg geen antwoord terug. Probeer het nog eens.";
    const clean = extractStep(aiText);
    history.push({ role: "model", text: aiText });
    addBubble(clean, "ai");
  } catch (err) {
    hideTyping();
    addBubble("⚠️ Geen verbinding met de server.", "ai");
  }
  reEnable();
}

function reEnable() {
  $("msgInput").disabled = false;
  $("sendBtn").disabled = false;
  $("msgInput").focus();
}

function startConversation() {
  $("chat").innerHTML = "";
  history = [];
  updateProgress(1);
  $("msgInput").disabled = false;
  $("sendBtn").disabled = false;
  sendToGemini(true);
}

function handleSend() {
  const input = $("msgInput");
  const text = input.value.trim();
  if (!text) return;
  addBubble(text, "me");
  history.push({ role: "user", text: text });
  input.value = "";
  sendToGemini(false);
}

$("sendBtn").addEventListener("click", handleSend);
$("msgInput").addEventListener("keydown", (e) => {
  if (e.key === "Enter") { e.preventDefault(); handleSend(); }
});

$("apiToggle").addEventListener("change", (e) => {
  apiEnabled = e.target.checked;
});

const appEl = $("app");
const savedTheme = localStorage.getItem("aibuddy_theme");
if (savedTheme === "gd") appEl.classList.add("theme-gd");
else if (savedTheme === "wl") appEl.classList.add("theme-wl");

function updateThemeBtns(variant) {
  $("themeGD").classList.toggle("active", variant === "gd");
  $("themeWL").classList.toggle("active", variant === "wl");
}

$("themeToggle").addEventListener("change", (e) => {
  if (e.target.checked) {
    const variant = localStorage.getItem("aibuddy_theme") || "wl";
    appEl.classList.remove("theme-gd", "theme-wl");
    appEl.classList.add("theme-" + variant);
    localStorage.setItem("aibuddy_theme", variant);
    $("themePicker").classList.remove("hidden");
    updateThemeBtns(variant);
  } else {
    appEl.classList.remove("theme-gd", "theme-wl");
    localStorage.removeItem("aibuddy_theme");
    $("themePicker").classList.add("hidden");
  }
});

$("themeGD").addEventListener("click", () => {
  appEl.classList.remove("theme-gd", "theme-wl");
  appEl.classList.add("theme-gd");
  localStorage.setItem("aibuddy_theme", "gd");
  updateThemeBtns("gd");
});

$("themeWL").addEventListener("click", () => {
  appEl.classList.remove("theme-gd", "theme-wl");
  appEl.classList.add("theme-wl");
  localStorage.setItem("aibuddy_theme", "wl");
  updateThemeBtns("wl");
});

$("openSettings").addEventListener("click", () => {
  $("promptArea").value = systemPrompt;
  $("apiToggle").checked = apiEnabled;
  const theme = localStorage.getItem("aibuddy_theme");
  $("themeToggle").checked = !!theme;
  if (theme) {
    $("themePicker").classList.remove("hidden");
    updateThemeBtns(theme);
  } else {
    $("themePicker").classList.add("hidden");
  }
  $("settings").classList.remove("hidden");
});
$("closeSettings").addEventListener("click", () => {
  $("settings").classList.add("hidden");
});
$("savePrompt").addEventListener("click", () => {
  const val = $("promptArea").value.trim();
  if (!val) return;
  systemPrompt = val;
  try { localStorage.setItem(STORAGE_KEY, val); } catch (e) {}
  $("settings").classList.add("hidden");
  startConversation();
});
$("resetPrompt").addEventListener("click", () => {
  systemPrompt = DEFAULT_PROMPT;
  try { localStorage.removeItem(STORAGE_KEY); } catch (e) {}
  $("promptArea").value = DEFAULT_PROMPT;
});

if (window.visualViewport) {
  window.visualViewport.addEventListener("resize", () => {
    const keyboardHeight = window.innerHeight - window.visualViewport.height;
    $("app").style.bottom = keyboardHeight + "px";
    scrollDown();
  });
}

$("msgInput").addEventListener("blur", () => {
  $("app").style.bottom = "0px";
});


initLogin();
